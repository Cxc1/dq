<template>
  <div class="toOtherLock">
    <my-header title="转到他人资产包">
    </my-header>
    <div class="main">
...
<script>
  ...
  export default {
    ...
    methods: {
      getInit(){
        getdata('/Supernode/recordTransferOtherLockInit', {
          token: this.$token,
          mark: this.$mark,
          ...
          this.temPlaceholder = '不得少于转出数量的' + this.initData.temp_min_rate + '%';
          ...
          if (this.money_type == 1) {
            this.topText = "*转到他人资产包时(放大3倍)将会在活动钱包最低扣除转入数额的" + this.initData.usable_min_rate + "" +
              "%以上(燃烧" + this.initData.usable_fee_rate + "%)，其余部分将在战绩钱包(燃烧" + this.initData.record_fee_rate + "%) " +
              "扣除，当活动钱包余额不足时将无法转账";
          } else if (this.money_type == 2) {
            this.topText = "*转到他人资产包时(放大3倍)将会在临时钱包最低扣除转入数额的" + this.initData.temp_min_rate + "" +
              "%(燃烧" + this.initData.usable_fee_rate + "%)，其余部分将在战绩钱包(燃烧" + this.initData.record_fee_rate + "%) " +
              "扣除，当临时钱包余额不足时将无法转账";
          } else if (this.money_type == 3) {
            this.topText = "*转到他人资产包时(放大3倍)将会在活动钱包最低扣除转入数额的" + this.initData.usable_min_rate  +
              "%以上(燃烧" + this.initData.usable_fee_rate + "%)，以及临时钱包最低扣除转入数额的"+ this.initData.temp_min_rate +
              "%以上(燃烧 "+ this.initData.temp_fee_rate + "%) ，其余部分将在战绩钱包(燃烧" + this.initData.record_fee_rate +
              "%)当活动钱包，临时钱包余额不足时将无法转账";
          }
        });
      },
