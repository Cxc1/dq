...
<script>
  ...
  export default {
    ...
    methods: {
      getInit() {
        getdata('/Supernode/recordTransferLockInit', {
          token: this.$token,
          mark: this.$mark,
          ...
          if (this.money_type == 1) {
            this.topText = "*转到自己资产包时(放大3倍)将会在活动钱包最低扣除转入数额的" + this.initData.usable_min_rate + "" +
              "%以上(燃烧" + this.initData.usable_fee_rate + "%)，其余部分将在战绩钱包(燃烧" + this.initData.record_fee_rate + "%) " +
              "扣除，当活动钱包余额不足时将无法转账";
          } else if (this.money_type == 2) {
            this.topText = "*转到自己资产包时(放大3倍)将会在临时钱包最低扣除转入数额的" + this.initData.temp_min_rate + "" +
              "%(燃烧" + this.initData.usable_fee_rate + "%)，其余部分将在战绩钱包(燃烧" + this.initData.record_fee_rate + "%) " +
              "扣除，当临时钱包余额不足时将无法转账";
            let temMax = div(this.initData.temp_money, (this.initData.temp_min_rate / 100)).toFixed(6);
            this.max_money = Number(this.max_money) > Number(temMax) ? temMax : this.max_money;
    ...
    mounted() {
      ...
      initNECaptcha(
        ...
        document.getElementById("assetsToUser_btn").addEventListener("click", function () {
          if (self.money != '' && self.usable_money != '' && self.user_pwd != '') {
            if (self.money > 0) {
              captchaIns.popUp();
            } else {
